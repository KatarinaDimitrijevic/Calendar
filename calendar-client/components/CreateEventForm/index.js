...
const CreateEventForm = ({ day }) => {
  ...
  const createMeeting = (meeting) => {
    ...
    fetch('http://localhost:7000/api/meetings', options).then((res) => res.json());
  };
  ...
  return (
    <div>
      <form onSubmit={(form) => onSubmitForm(form)}>
        ...
        <button className={styles.button} onClick={(form) => resetForm(form)}>
          Cancel
        </button>
        <button className={styles.button} onClick={(form) => closeForm(form)}>
          Close
        </button>
